...
def transfer(r, frequencies, inc_deg, m, mdot_E, a, h_x, L_x, A):
    ...
    def _inside_of_integral(r, frequencies, inc, m, mdot_E, a, h_x, L_x, A):
        '''Calculates dL_nu/dr'''
        x = 2*pi*2*h*frequencies**3*G**2*M_sun**2*m**2/c**6
...
transferfunction = np.zeros(freq_data_points)
...
#here we loop through all our radii to find the transfer function, the extra term after the loop is due to the
#final term's dr being different to calculate
#'''
for i in range(len(transferfunction) -1):
    transferfunction[i] = transfer(radii[i], frequencies, inclination, mass_BH, mdot_eddington, a, h_x, L_x, A)\
                    *4*pi*radii[i] * (radii[i+1] - radii[i])
transferfunction[-1] = transfer(radii[-1], frequencies, inclination, mass_BH, mdot_eddington, a, h_x, L_x, A)\
                    *4*pi*radii[-1] * (radii[-1]*(radii[1]/radii[0]) - radii[-1])
'''                 
for i in range(len(transferfunction) -1):
    transferfunction[i] = transfer(radii[i], frequencies, inclination, mass_BH, mdot_eddington, a, h_x, L_x, A)\
                    *4*pi*radii[i]
...
pylab.title("Transfer Function Spectrum for log10 Frequency Range [" + "{:.2g}".format(np.log10(freq_low)) +\
           ", " + "{:g}".format(np.log10(freq_high)) + "] Hz", fontsize = 16)
pylab.xlabel("Radius (R$_g$)", fontsize = 14)
pylab.ylabel("Transfer Function", fontsize = 14)
pylab.plot(radii, transferfunction / L_sun, label = "Reprocessing of Disk with Radius of Disk")
pylab.xscale("log")
...
pylab.figure(figsize = (10, 8))
pylab.title("Transfer Function for log10 Frequency Range [" + "{:.2g}".format(np.log10(freq_low)) +\
           ", " + "{:g}".format(np.log10(freq_high)) + "] Hz", fontsize = 16)
pylab.xlabel("Lag (s)", fontsize = 14)
pylab.ylabel("Transfer Function", fontsize = 14)
pylab.plot(times, transferfunction / L_sun, label = "Reprocessing of Disk with Time")
pylab.xscale("log")
pylab.xticks(fontsize=12)
pylab.yticks(fontsize=12)
pylab.legend(fontsize = 14)
...
#print("Total extra luminosity from x-ray heating " + "{:e}".format(integrate(transferfunction, x=radii) / L_sun) + " L_☉")