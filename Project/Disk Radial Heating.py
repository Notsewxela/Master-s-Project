...
def T_new(r, m, mdot_E, a, h_x, L_x, A):
    ...
    xray_heating_term = const2 * (1-A) * L_x * h_x * (1-np.exp(-(1-(r_isco(a)/r)**(1/2))**(4/5))) / (m**2 * (h_x**2+r**2)**(3/2))
...
def dLdr_diff(r, frequencies, inc_deg, m, mdot_E, a, h_x, L_x, A):
    '''Calculates the disk radial intensity for a disk'''
    #A = 1-np.tanh(r/r_isco(a)/100)
    inc = inc_deg*pi/180
...
def dLdr_without_heating(r, frequencies, inc_deg, m, mdot_E, a, h_x, L_x, A):
    '''Calculates the disk radial intensity for a disk'''
    #A = 1-np.tanh(r/r_isco(a)/100)
    inc = inc_deg*pi/180
    def _inside_of_integral(r, frequencies, inc, m, mdot_E, a, h_x, L_x, A):
        '''Calculates dL_nu/dr'''
        x = 4*pi*2*h*frequencies**3*G**2*M_sun**2*m**2/c**6 * 2*pi*r
        y = 1/(np.exp(h*frequencies/(k_B*T(r, m, mdot_E, a, h_x, L_x, A)))-1)
        return x*y
...
def dLdr_with_heating(r, frequencies, inc_deg, m, mdot_E, a, h_x, L_x, A):
    '''Calculates the disk radial intensity for a disk'''
    #A = 1-np.tanh(r/r_isco(a)/100)
    inc = inc_deg*pi/180
    def _inside_of_integral(r, frequencies, inc, m, mdot_E, a, h_x, L_x, A):
        '''Calculates dL_nu/dr'''
        x = 4*pi*2*h*frequencies**3*G**2*M_sun**2*m**2/c**6 * 2*pi*r
        y = 1/(np.exp(h*frequencies/(k_B*T_new(r, m, mdot_E, a, h_x, L_x, A)))-1)
        return x*y
...
def tau(r, h_x, m):
    '''Claculates the extra time in seconds that light must travel from a radius r of the
...
dLdrsdiff = np.zeros(freq_data_points)
dLdrswithheat = np.zeros(freq_data_points)
dLdrswithoutheat = np.zeros(freq_data_points)
...
for i in range(len(dLdrsdiff)):
    dLdrsdiff[i] = dLdr_diff(radii[i], frequencies, inclination, mass_BH, mdot_eddington, a, h_x, L_x, A)
    dLdrswithheat[i] = dLdr_with_heating(radii[i], frequencies, inclination, mass_BH, mdot_eddington, a, h_x, L_x, A)
    dLdrswithoutheat[i] = dLdr_without_heating(radii[i], frequencies, inclination, mass_BH, mdot_eddington, a, h_x, L_x, A)


pylab.figure(figsize = (10, 8))
pylab.title("Disk Radial Spectrum for log10 Frequency Range [" + "{:.2g}".format(np.log10(freq_low)) +\
           ", " + "{:g}".format(np.log10(freq_high)) + "] Hz", fontsize = 16)
pylab.xlabel("Radius (R$_g$)", fontsize = 14)
pylab.ylabel("$\\frac{\\partial L}{\\partial r}$ (Wm$^{-1}$)", fontsize = 14)
pylab.plot(radii, dLdrswithoutheat, label = "No X-Ray Reprocessing")
pylab.plot(radii, dLdrswithheat, label = "X-Ray Reprocessing")
pylab.plot(radii, dLdrsdiff, label = "Difference")
pylab.xscale("log")
...
print("Total disk luminosity without x-ray heating " + "{:e}".format(integrate(dLdrswithoutheat, x=radii) / L_sun) + " L_☉")
print("Total disk luminosity with x-ray heating " + "{:e}".format(integrate(dLdrswithheat, x=radii) / L_sun) + " L_☉")
print("Total extra luminosity from x-ray heating " + "{:e}".format(integrate(dLdrsdiff, x=radii) / L_sun) + " L_☉")

dLdtausdiff = dLdrsdiff*drdtau(radii, h_x, mass_BH)
#radii = np.insert(radii, 0, np.linspace(1e-7, radii[0], num = 50, endpoint = False))
#dLdtausdiff = np.insert(dLdtausdiff, 0, 1e-7+np.zeros(50))
times = tau(radii, h_x, mass_BH)

pylab.figure(figsize = (10, 8))
pylab.title("Disk Lag Spectrum for log10 Frequency Range [" + "{:.2g}".format(np.log10(freq_low)) +\
           ", " + "{:g}".format(np.log10(freq_high)) + "] Hz", fontsize = 16)
pylab.xlabel("Lag (s)", fontsize = 14)
pylab.ylabel("$\\frac{\\partial L}{\\partial \\tau}$ (Ws$^{-1}$)", fontsize = 14)
pylab.plot(times, dLdtausdiff, label = "Difference")
pylab.xscale("log")
pylab.xticks(fontsize=12)
pylab.yticks(fontsize=12)
pylab.legend(fontsize = 14)
...
print("Total extra luminosity from x-ray heating " + "{:e}".format(integrate(dLdtausdiff, x=times) / L_sun) + " L_☉")