...
def FWHM(old_x, old_y, bins, trim, peaks, height = 0.5):
    ...
    widths, width_heights, left_ips, right_ips = peak_widths(y, peaks, rel_height=height)
    widths *= dx
...
axs[0].set_title("Binned Cross-Correlation Functions", fontsize = 16)
...
for i, f in enumerate(filenames):
    ...
    sorted_taus, sorted_UDCCF = UDCCF_calc(xmodel, ymodel, xdata, ydata)
    ...
    axs[i].errorbar(xpositions, DCCF, yerr=DCCF_errs, ls = "", marker = ".", markersize = 2, color = colors[i])
    
    
    peaks = find_peaks(DCCF)[0]
    axs[i].plot(xpositions[peaks], DCCF[peaks], "rx")

    heights = [0.25, 0.5, 0.75]
    labels = ["fwqm", "fwhm", "fw3qm"]
    for j, h in enumerate(heights):
        widths, width_heights, left_ips, right_ips = FWHM(xpositions, DCCF, bin_num1, trim, peaks, height = h)
        print(f + " CCF " + labels[j] + " = " + "{:.2f}".format(widths[0]) + " $\pm$ " +\
              "{:.2f}".format(FWHM(xpositions, DCCF, bin_num1, trim, peaks, height = h+0.05)[0][0] - FWHM(xpositions, DCCF, bin_num1, trim, peaks, height = h-0.05)[0][0]))
...
#plt.savefig("cross-correlation.pdf", bbox_inches = "tight")