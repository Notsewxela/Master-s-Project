...
dtaus = np.zeros((len(radii), len(angles)))
...
for i in range(len(xs)):
    ...
    try:
        dtaus[i][0:phi_data_points] = abs(D(radii[i+1], mass_BH, h_X, angles, inclination)/c - D(radii[i], mass_BH, h_X, angles, inclination)/c)
    except:
        dtaus[i][0:phi_data_points] = abs(D(radii[i], mass_BH, h_X, angles, inclination)/c - D(radii[i-1], mass_BH, h_X, angles, inclination)/c)


pylab.figure(figsize = (10, 8*np.cos(inclination*pi/180)))
h=pylab.contourf(xs, ys, taus, 100, cmap = pylab.cm.gnuplot_r)
...
pylab.savefig("inclined_lag_contour" + str(inclination) +  ".png", bbox_inches="tight")
pylab.show()
...
pylab.figure(figsize = (10, 8*np.cos(inclination*pi/180)))
h=pylab.contourf(xs, ys, dtaus, 100, cmap = pylab.cm.gnuplot_r)
pylab.xlabel("x (R$_{g}$)", fontsize = 14)
pylab.ylabel("Projectecd y (R$_{g}$)", fontsize = 14)
pylab.title("d$\\tau$ Contour Plot", fontsize = 18)
pylab.arrow(0, 0, 0, -radii[-1]/7, head_width = radii[-1]/20*np.cos(inclination*pi/180), color="white")
pylab.text(radii[-1]*4/7, radii[-1]*np.cos(inclination*pi/180)*0.85, "Inclination: " + str(inclination) + "°", fontsize=12)
cb=pylab.colorbar(h)
cb.ax.tick_params(labelsize=14)
cb.set_label("Lag Difference (s)", fontsize=14)
