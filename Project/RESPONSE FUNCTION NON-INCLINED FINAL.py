...
#allows us to plot multiple spins on same plot
#spins = [1, 0, -1]
#heights = [3, 9, 100]

peak_wavelengths = np.array([1928, 2246, 2600, 3465, 4392, 5468])*1e-10
FWHMs = np.array([657, 498, 693, 785, 975, 769])*1e-10
filters = ["W2", "M2", "W1", "U", "B", "V"]
...
for i in range(len(peak_wavelengths)):
    ...
    freq_low =  c/(peak_wavelengths[i] + FWHMs[i]/2) #1e5#1.3*10**(15) #
    freq_high = c/(peak_wavelengths[i] - FWHMs[i]/2) #10**(17.5)#1.9*10**(15) #
    ...
    if spin >= 0:
        extra_space = " "
    #pylab.plot(taus, responses, label = "$a$ = " + extra_space + str(spin))
    pylab.plot(taus, responses, label = filters[i])
    print("Total extra luminosity from disk response spin " + str(spin) + ": {:e}".format(integrate(responses, x = taus) / L_sun) + " L_☉")
    np.savetxt("Data Files/response_" + filters[i] + ".dat", (taus, responses))
    #np.savetxt("Data Files/response_spin_" + str(spin) + "_" + "{:.2E}".format(freq_low) + "-" + "{:.2E}".format(freq_high) + ".dat", (taus, responses))
...
#pylab.savefig("multiband_responses_comparisons.pdf", bbox_inches = 'tight')
pylab.show()
    


#np.savetxt("Data Files/response.dat", (taus, responses))
