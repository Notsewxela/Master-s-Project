...
def T_new_bad(r, m, mdot_E, a, h_x, L_x, A):
    '''Calculates the new effective temperature of the disk after it has absorbed
    some x-rays from the xray source. Same inputs as T with the addition of
    height of x ray source above disk (in gravitational units), the xray luminosity
    (in watts) and the albedo.'''
...
phi_data_points = 100
phis = np.linspace(0, pi, phi_data_points + 1) #as the disk is symmetric we only need to do 0->pi and double the result


taus = np.zeros(radii_data_points*phi_data_points) #stored smartly xd
transfer = np.zeros(radii_data_points*phi_data_points)
transfer2 = np.zeros(radii_data_points*phi_data_points)
...
for i in range(radii_data_points):
    ...
    T3 = T_new_bad(radii[i], mass_BH, mdot_eddington, a, h_x, L_x, A)
    Bs_tot2 = integrate(B_nu(T3, frequencies) - B_nu(T2, frequencies), x = frequencies)
    Bs_tot = integrate(B_nu(T2, frequencies) - B_nu(T1, frequencies), x = frequencies)
    #Bs_tot = B_nu(T2, 10**17) - B_nu(T1, 10**17)
    for j in range(phi_data_points):
        transfer[phi_data_points*i+j] = 4*pi*Bs_tot * (G*M_sun*mass_BH)**2/c**4\
            * radii[i] * (radii[i+1] - radii[i]) * c\
            / (D(radii[i+1], mass_BH, h_x, phis[j+1], inclination) - D(radii[i], mass_BH, h_x, phis[j], inclination))
            
        transfer2[phi_data_points*i+j] = 4*pi*Bs_tot2 * (G*M_sun*mass_BH)**2/c**4\
            * radii[i] * (radii[i+1] - radii[i]) * c\
            / (D(radii[i+1], mass_BH, h_x, phis[j+1], inclination) - D(radii[i], mass_BH, h_x, phis[j], inclination))    
            
        taus[phi_data_points*i+j] = D(radii[i], mass_BH, h_x, phis[j], inclination) / c

transfer *= 2*pi*np.cos(inclination * pi/180) #whole disk is 2pi
transfer2 *= 2*pi*np.cos(inclination * pi/180) 

tausinds = taus.argsort()
sorted_taus = taus[tausinds[::1]]
sorted_transfer = transfer[tausinds[::1]]
sorted_transfer2 = transfer2[tausinds[::1]]
...
pylab.plot(sorted_taus, sorted_transfer, label="Optical Depth Effect Considered")
#pylab.plot(sorted_taus, sorted_transfer2, label="Optical Depth Effect Not Considered")
#pylab.plot(sorted_taus, sorted_transfer2-sorted_transfer, label="eek2")
pylab.xscale("log")
#pylab.yscale("log")
pylab.xticks(fontsize=12)
pylab.yticks(fontsize=12)
pylab.legend(fontsize = 14)
pylab.savefig("Response-Function-(Good-Bad)-Comparison.pdf")
pylab.show()
